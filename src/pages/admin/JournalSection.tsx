...
export const JournalSection: React.FC = () => {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      if (defaultEnv) {
        const progData = await getMontessoriProgress({ environmentId: defaultEnv });
  ...
  const handleOpenTrackerCell = (student: StudentItem, tracker: any, categoryName: string) => {
    const key = `${tracker.id}_${student.id}`;
    const current = dailyTrackerLogs[key];
    setDrawerStudent(student);
    setDrawerItem({
      type: 'tracker',
      id: tracker.id,
      title: tracker.name,
      subtitle: `Hábito Diario • ${categoryName}`,
      description: tracker.description || 'Parámetro de seguimiento y registro de rutina.'
    });
    ...
    try {
      if (drawerItem.type === 'lesson') {
        ...
        setDailyTrackerLogs(prev => ({
          ...prev,
          [key]: { value: drawerStatus, notes: drawerNotes.trim() }
        }));
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Buscar lección o alumno..."
                          ...
                          <React.Fragment key={sub.id}>
                            {(sub.items || []).map(item => (
                              <tr key={item.id} className="hover:bg-forest/5 transition-colors">
                                ...
                                {currentStudents.map(student => {
                                  const key = `${item.id}_${student.id}`;
      ...
      {drawerStudent && drawerItem && (
        ...
        >
          <div className="space-y-5 p-4 text-xs font-body">
            ...
            {/* Observation Notes */}
            <div className="space-y-1.5">
              <label className="block text-forest font-bold">Notas de Observación / Guía:</label>
              <textarea
                rows={3}
                value={drawerNotes}
                onChange={(e) => setDrawerNotes(e.target.value)}
                placeholder="Escribe observaciones cualitativas, desafíos o logros del infante..."
                className="w-full p-2.5 rounded-xl border border-forest/20 text-forest bg-white text-xs shadow-2xs focus:outline-none focus:ring-1 focus:ring-forest leading-relaxed"
              />
            </div>
          </div>
        </SlideOverDrawer>

...
export const MontessoriSection: React.FC = () => {
 ...
 // Montessori AI Voice Logger State
 ...
 // Mobile Tabs Horizontal Scroll Handlers
 ...
 setCanScrollLeftTabs(scrollLeft > 5);
 setCanScrollRightTabs(scrollLeft < scrollWidth - clientWidth - 5);
 }
 };
 ...
 const matchesAnyStudent = currentEnvStudents.some(s => s.full_name.toLowerCase().includes(q));
 ...
 return currentEnvStudents.some(st => {
 const key = `${item.id}_${st.id}_${attendanceDate}`;
 const keyWithoutDate = `${item.id}_${st.id}`;
 const log = dailyTrackerLogs[key] || dailyTrackerLogs[keyWithoutDate];
 return (
 ...
 }, [trackerCategories, searchQuery, currentEnvStudents, dailyTrackerLogs, attendanceDate]);
 ...
 // Reload progress, observations and attendance when selected environment or date changes
 useEffect(() => {
 if (selectedEnvId) {
 ...
 }, [selectedEnvId, attendanceDate, trackerCategories]);
 ...
 {/* MOBILE DEDICATED FULL-WIDTH DATE FILTER */}
 <div className="sm:hidden w-full flex items-center justify-between p-2.5 rounded-2xl bg-white border border-forest/10 shadow-xs">
 <div className="flex items-center gap-2 text-forest">
 <Calendar className="w-4 h-4 text-forest shrink-0" />
 <span className="text-xs font-bold">Fecha de Registro:</span>
 </div>
 <input
 type="date"
 value={attendanceDate}
 onChange={(e) => setAttendanceDate(e.target.value)}
 className="bg-forest/5 text-forest text-xs font-bold px-3 py-1.5 rounded-xl border border-forest/10 focus:outline-none cursor-pointer"
 />
 </div>
 ...
 className="sm:hidden absolute left-0 top-1/2 -translate-y-1/2 z-20 w-7 h-7 rounded-full bg-white/95 dark:bg-gray-900/95 border border-forest/20 text-forest shadow-md flex items-center justify-center -ml-1 cursor-pointer"
 >
 ...
 className="sm:hidden absolute right-0 top-1/2 -translate-y-1/2 z-20 w-7 h-7 rounded-full bg-white/95 dark:bg-gray-900/95 border border-forest/20 text-forest shadow-md flex items-center justify-center -mr-1 cursor-pointer"
 >
 ...
 className="bg-transparent sm:bg-white p-0 sm:p-3 rounded-none sm:rounded-3xl border-0 sm:border border-forest/10 shadow-none sm:shadow-xs flex items-center justify-start sm:justify-between gap-2 overflow-x-auto no-scrollbar scroll-smooth whitespace-nowrap active:cursor-grab select-none touch-pan-x"
 >
 {/* Segmented Tabs (Loose elements on mobile, container on sm) */}
 <div className="flex items-center gap-1.5 p-0 sm:p-1 bg-transparent sm:bg-forest/5 rounded-none sm:rounded-2xl border-0 sm:border border-forest/10 shrink-0">
 <button
 type="button"
 onClick={() => setTrackingType('work_cycle')}
 className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 cursor-pointer shrink-0 shadow-xs sm:shadow-none ${
 trackingType === 'work_cycle'
 ? 'bg-[#C4661F] text-white shadow-xs scale-[1.01]'
 : 'bg-white sm:bg-transparent text-forest/70 hover:text-forest hover:bg-white/60 border border-forest/10 sm:border-0'
 }`}
 >
 ...
 {/* Internal Observation Notes with Voice Dictation */}
 <VoiceNoteTextarea
 label="Notas de Observación / Guía (Opcional):"
 value={modalNotes}
 onChange={setModalNotes}
 rows={4}
 placeholder="Escribe o dicta observaciones cualitativas, detalles sobre el periodo sensible, interés o retos del niño..."
 />
 </div>
 </SlideOverDrawer>
 )}
 ...
 {/* Public Note (For Parents) with Voice Dictation */}
 <VoiceNoteTextarea
 label={
 ...
 onChange={setModalTrackerPublicNote}
 rows={3}
 placeholder="Comentario o felicitación que los tutores podrán ver en su portal familiar..."
 />

 {/* Private Note (Internal Guías & Dirección) with Voice Dictation */}
 <VoiceNoteTextarea
 label={
 <span className="text-xs font-bold text-forest uppercase tracking-wider flex items-center gap-1.5">
 ...
 onChange={setModalTrackerPrivateNote}
 rows={3}
 ...
 if (trackingType !== 'trackers') {
 setTrackingType('lessons');
 }
 ...
 initialTargetType={trackingType === 'trackers' ? 'tracker' : 'lesson'}
 environmentId={selectedEnvId}
 preselectedStudentId={voiceLoggerStudentId}
 selectedDate={attendanceDate}
 onTargetTypeChange={(target) => {
 setTrackingType(target === 'tracker' ? 'trackers' : 'lessons');
 }}
 onSaved={(target) => {
 setTrackingType(target === 'tracker' ? 'trackers' : 'lessons');
 loadData();

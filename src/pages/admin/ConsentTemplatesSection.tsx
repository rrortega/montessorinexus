...
export const ConsentTemplatesSection: React.FC = () => {
  ...
  const handleOpenEdit = (item: ConsentTemplateItem) => {
    if (!isOwnerOrAdmin) return;
    if (item.isDefault) {
      toast.info('Los consentimientos predeterminados del sistema no se pueden editar.');
      return;
    }
  ...
  const handleToggleActive = async (item: ConsentTemplateItem) => {
    if (!isOwnerOrAdmin) return;
    if (item.isDefault) {
      toast.info('Los consentimientos predeterminados del sistema permanecen activos por normativa.');
      return;
    }
    try {
  ...
  const handleExecuteDelete = async () => {
    ...
    const target = templates.find(c => c.id === confirmDelete.id);
    if (target?.isDefault) {
      toast.error('No se pueden eliminar las plantillas predeterminadas del sistema.');
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="space-y-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {item.isDefault ? (
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-forest/10 text-forest border border-forest/20 inline-flex items-center gap-1">
                    ...
                    {isOwnerOrAdmin && (
                      <div className="flex items-center gap-1 shrink-0">
                        {item.isDefault ? (
                          <span
                            className="text-[11px] font-semibold text-muted-foreground/80 bg-forest/5 border border-forest/10 px-2.5 py-1 rounded-xl inline-flex items-center gap-1 cursor-default select-none"
                            title="Plantilla fija del sistema (protegida contra edición y eliminación)"
                          >
                            <Lock className="w-3.5 h-3.5 text-forest/70" />
                            <span className="hidden sm:inline">Fijo del sistema</span>
                          </span>
                        ) : (
                          <>
                            <button

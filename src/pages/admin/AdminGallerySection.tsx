...
export const AdminGallerySection: React.FC = () => {
  ...
  const handleScanAllConsents = async () => {
    ...
    try {
      ...
      toast.success(`🛡️ Escaneo de consentimientos completado para ${res.total} fotografía(s)`);
      loadData(true);
    } catch (e: any) {
  ...
  const handleScanSingleConsent = async (imgId: string, e?: React.MouseEvent) => {
    ...
    try {
      ...
      if (res.hasConsentIssues) {
        toast.warning(`⚠️ Se detectaron alumnos sin consentimiento. Se generó versión difuminada.`);
      } else {
            ...
            {previewImage.blurred_src && isOwnerOrAdmin && (
              <div className="flex items-center justify-between p-2.5 rounded-2xl bg-forest/5 border border-forest/15 flex-wrap gap-2">
                ...
                <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-forest/15 shadow-3xs">
                  ...
                  >
                    🛡️ Vista Web Protegida (Difuminada)
                  </button>
                </div>
              </div>
            )}
            ...
            <div className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-3xs space-y-4">
              ...
              <div className="pt-3 border-t border-slate-200/60 space-y-2.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  ...
                  {/* Toggle Web Pública */}
                  <div
                    onClick={() => setShowOnWeb(!showOnWeb)}
                    className={`p-3 rounded-xl border transition-all flex items-center justify-between gap-2.5 cursor-pointer select-none ${showOnWeb
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-slate-500 block">Landing page y visitantes</span>
                      </div>
                    </div>
                    <Switch
                      checked={showOnWeb}
                  ...
                  <div
                    ...
                    className={`p-3 rounded-xl border transition-all flex items-center justify-between gap-2.5 cursor-pointer select-none ${showOnPortal
                        ? 'bg-blue-50/50 border-blue-300/80 shadow-3xs'
                        : 'bg-slate-50 border-slate-200 hover:border-slate-300'
                      }`}
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-slate-500 block">Comunidad de familias</span>
                      </div>
                    </div>
                    <Switch

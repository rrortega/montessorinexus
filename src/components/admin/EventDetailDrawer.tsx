...
export const EventDetailDrawer: React.FC<EventDetailDrawerProps> = ({
  ...
  const canEdit = isOwnerOrAdmin || permissions.includes('events:write') || permissions.includes('events') || currentEvent?.hosts?.some(h => h.userId === user?.id);
        ...
        <div className="flex items-center justify-between w-full">
          ...
          {canEdit ? (
            <button
              ...
              className="px-4 py-2 text-xs font-bold text-forest bg-forest/5 hover:bg-forest/10 rounded-xl transition-colors flex items-center gap-1.5"
            >
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-6 py-2.5 bg-forest hover:bg-forest/90 text-white rounded-xl text-xs font-bold shadow-xs transition-all hover:scale-105 active:scale-95"
          >
            Cerrar
          </button>
        </div>

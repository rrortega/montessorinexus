...
export const EventFormDrawer: React.FC<EventFormDrawerProps> = ({
      ...
      if (event.slots && event.slots.length > 0) {
        const mappedSlots: FormSlot[] = event.slots.map(s => {
          ...
          return {
            id: s.id,
            name: s.name || '',
            date: sStart.toISOString().split('T')[0],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim() || !categoryId || !startDate || !endDate) {
          ...
          <div className="flex items-center gap-2">
            ...
            {activeTab !== 'schedule' ? (
              <button
                type="button"
                onClick={() => {
                  if (activeTab === 'general') setActiveTab('audience');
              ...
              >
                ...
                <span>{loading ? 'Guardando...' : event ? 'Guardar Cambios' : 'Publicar Evento'}</span>
              </button>
            )}
          </div>
        </div>
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-3.5 py-2.5 rounded-xl border border-forest/20 text-xs focus:outline-hidden focus:ring-2 focus:ring-forest bg-white leading-relaxed"
              />
            </div>
